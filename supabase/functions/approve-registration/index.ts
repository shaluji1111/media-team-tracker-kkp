...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: request, error: requestError } = await supabase
      .from('self_registration_requests')
      .select('*')
      .eq('id', body.registration_id)
      .maybeSingle();
    if (requestError || !request) throw new Error('Registration request not found.');
    ...
    await supabase.from('audit_events').insert({
      ...
      action: 'user_created',
      metadata: { source: 'self_registration', jsid, role: body.role },
    });
